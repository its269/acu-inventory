...
export default function IncomingPOPage() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("inc_po_user_inputs");
        if (saved) {
            try {
                const parsed = JSON.parse(saved);
                setTimeout(() => setUserInputs(parsed), 0);
            } catch (e) {
                console.error("Failed to parse inc_po_user_inputs", e);
            }
        }
    }, []);
    ...
    useEffect(() => {
        if (!isInitialMount.current) {
            localStorage.setItem("inc_po_user_inputs", JSON.stringify(userInputs));
        }
    }, [userInputs]);
        ...
        <div className="db-root" style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', padding: '2rem' }}>
            <main className="db-main" style={{ flex: 1, minWidth: 0, padding: 0, margin: 0, maxWidth: 'none' }}>
                <div className="db-page-title">
                    ...
                    <p>Track incoming shipments and manage ETA for open orders.</p>
                </div>
                            ...
                            {loading && orders.length === 0 ? (
                                <tr><td colSpan={8} className="si-loading-cell">
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                                        ...
                                        <span>Fetching incoming orders...</span>
                                    </div>
                                </td></tr>
                            ) : orders.length === 0 ? (
                                <tr><td colSpan={8} className="si-empty-cell">No incoming orders found.</td></tr>
                            ) : orders.map(po => {
            ...
            <aside className="po-right-panel">
                <div className="po-summary-card">
                    ...
                    <div className="po-summary-item">
                        <span className="po-summary-label">Total Incoming</span>
                ...
                <div className="po-info-box">
                    ...
                    <p className="po-info-text">
                        Use this module to coordinate with logistics. By entering the <strong>ETA</strong>, you help the warehouse team prepare for incoming stocks. 
                    </p>
