        ...
        <div className="si-root">
            <div className="si-main">
                ...
                <div className="db-page-title">
                    <h1>Stock Items</h1>
                    <p>All stock items across every branch — sourced from Supabase</p>
                </div>

                {/* Summary chip */}
                <div className="si-summary-row">
                    <span className="si-total-chip">
                        {loading ? "Loading…" : `${totalCount.toLocaleString()} total items`}
                    </span>
                </div>
                ...
                <div className="db-table-wrap">
                    <table className="db-table">
                        ...
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={9} className="si-loading-cell">Loading…</td>
                                </tr>
                            ) : items.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="si-empty-cell">No stock items found.</td>
                                </tr>
                            ) : items.map((item, idx) => {
                                const status = stockStatus(item.onHand);
                                return (
                                    <tr key={`${item.inventoryId}-${item.branch}-${idx}`}>
                                        <td><span className="si-id-chip">{item.inventoryId}</span></td>
                                        <td>{item.description}</td>
                                        <td>{item.itemClass}</td>
                                        <td>{item.branch}</td>
                                        <td>{item.site}</td>
                                        <td><strong>{Number(item.onHand).toLocaleString()}</strong></td>
                                        <td>{Number(item.available).toLocaleString()}</td>
                                        <td>₱{Number(item.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                        <td>
                                            <span className={`db-status-badge ${status.cls}`}>{status.label}</span>
                                        </td>
                                    </tr>
                                );
                            })}
